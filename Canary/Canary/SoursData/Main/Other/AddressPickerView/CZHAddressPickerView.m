...
@property (nonatomic,strong)   NSArray * bankAray;
 ...
 * provinceBlock : 回调省份
 */
+ (instancetype)provincePickerViewWithtype:(NSString * )type  ProvinceBlock:(void(^)(NSString *province))provinceBlock
{
        return [CZHAddressPickerView addressPickerViewWithProvince:nil city:nil area:nil type:type  provinceBlock:provinceBlock cityBlock:nil areaBlock:nil showType:CZHAddressPickerViewTypeProvince];
}
/**
// * 显示省份和市级
// * cityBlock : 回调省份和城市
 ...
 * areaBlock : 回调省份城市和区域
 */
+ (instancetype)areaPickerViewWithProvince:(NSString *)province city:(NSString *)city area:(NSString *)area  type:(NSString *)type areaBlock:(void(^)(NSString *province, NSString *city, NSString *area))areaBlock {
    return [CZHAddressPickerView addressPickerViewWithProvince:province city:city area:area  type:type provinceBlock:nil cityBlock:nil areaBlock:areaBlock showType:CZHAddressPickerViewTypeArea];
}




+ (instancetype)addressPickerViewWithProvince:(NSString *)province city:(NSString *)city area:(NSString *)area type:(NSString *)type  provinceBlock:(void(^)(NSString *province))provinceBlock cityBlock:(void(^)(NSString *province, NSString *city))cityBlock areaBlock:(void(^)(NSString *province, NSString *city, NSString *area))areaBlock  showType:(CZHAddressPickerViewType)showType{
...
- (instancetype)init {
    if (self = [super init]) {
        ...
        [self czh_setView];
        self.bankAray = @[@"国家开发银行",@"中国工商银行",@"中国农业银行",@"中国银行",@"中国建设银行",@"中国邮政储蓄银行",@"交通银行",@"招商银行",@"上海浦东发展银行",@"兴业银行",@"华夏银行",@"广东发展银行",@"中国民生银行",@"中信银行",@"中国光大银行",@"恒丰银行",@"浙商银行",@"渤海银行",@"平安银行",@"北京银行",@"上海农村商业银行",@"玉溪市商业银行",@"江苏银行",@"杭州银行",@"南京银行",@"宁波银行",@"长沙银行",@"成都银行",@"大连银行",@"青岛银行",@"内蒙古银行"];
    }
    return self;
}
...
- (void)czh_setView {
    ...
    containView.frame = CGRectMake(0, ScreenHeight, ScreenWidth, CZH_ScaleHeight(200));
...
- (NSInteger)numberOfComponentsInPickerView:(UIPickerView *)pickerView
{
    ...
    if ([self.typeStr isEqualToString:@"bank"]) {
        return 1;
    }else
    {
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
    if (component == 0) {//选择省
        ...
        if (self.showType == CZHAddressPickerViewTypeProvince) {
            if ([self.typeStr isEqualToString:@"bank"]) {
                self.selectProvince = self.bankAray[row];
                self.selectCity = @"";
                self.selectArea = @"";
            }else
            {
