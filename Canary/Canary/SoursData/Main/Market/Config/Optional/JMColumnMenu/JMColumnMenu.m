...
/** 添加的数组*/
@property (nonatomic, strong) NSString *addArr;
/** 减少的数组*/
@property (nonatomic, strong) NSString *reductionArr;
@end
...
//}
//
//- (NSMutableArray *)reductionArr{
//    if (!_reductionArr) {
//        _reductionArr = [NSMutableArray array];
//    }
//    return _reductionArr;
//}
...
- (instancetype)initWithTagsArrM:(NSMutableArray *)tagsArrM OtherArrM:(NSMutableArray *)otherArrM Type:(JMColumnMenuType)type Delegate:(id<JMColumnMenuDelegate>)delegate{
    if (self = [super init]) {
        ...
        for (int i = 0; i < tagsArrM.count; i++) {
            ...
            model.title = tagsArrM[i][@"title_cn"];
            model.title_cn = tagsArrM[i][@"title"];
            model.type = type;
        ...
        for (NSDictionary * dic in otherArrM) {
            JMColumnMenuModel *model = [[JMColumnMenuModel alloc] init];
//            model.title = title;
            model.title = dic[@"title_cn"];
            model.title_cn = dic[@"title"];
            if (self.type == JMColumnMenuTypeTouTiao) {
                model.showAdd = YES;
            }
            model.type = type;
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0) {
        ...
        self.reductionArr = model.title_cn;
...
- (void)updateBlockArr {
    ...
    if ([self.delegate respondsToSelector:@selector(columnMenuTagsArr:OtherArr:AddString:ReductionStirng:)]) {
        [self.delegate columnMenuTagsArr:tempTagsArrM OtherArr:tempOtherArrM AddString:self.addArr ReductionStirng:self.reductionArr];
    }
