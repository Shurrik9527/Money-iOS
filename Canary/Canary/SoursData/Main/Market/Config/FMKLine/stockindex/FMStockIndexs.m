...
+(NSMutableDictionary*)getKDJMap:(NSArray*)m_kData{
    // 默认随机值
    int m_iParam[] = {[[LTUtils getSeting:@"KDJ_N"] intValue], 3, 3};
    int n1 = m_iParam[0];
    int n2 = m_iParam[1];
    int n3 = m_iParam[2];
...
+(float)RSI:(int)day N:(int)N list:(NSArray*)list ly:(float*)ly ly1:(float*)ly1 {
    ...
    // DaysChartModel *m = (DaysChartModel*)[list objectAtIndex:day];
    DaysChartModel *m = [[DaysChartModel alloc] initWithDic:[list objectAtIndex:day]];
    ...
    if (day==N) {
        ...
        float c2 = 0;
        for (int i=1; i<N; i++) {
            DaysChartModel *dcm = [[DaysChartModel alloc] initWithDic:[list objectAtIndex:i]];
...
+(CGFloat)REF:(NSString *)key day:(NSInteger)day list:(NSArray *)list
{
    ...
    NSDictionary *dic = [list objectAtIndex:day];
    CGFloat value = [[dic objectForKey:key] floatValue];
    return value;
}
/*
...
#pragma mark - 传入priceslist算出对应的一个item
//array必传
...
{
    if (!KDJ) {
        KDJ=[self getKDJMap:array];
    }
    // 计算其他指标
    int SMA_N1 = [[LTUtils getSeting:@"SMA_N1"] intValue];
    int SMA_N2 = [[LTUtils getSeting:@"SMA_N2"] intValue];
    int SMA_N3 = [[LTUtils getSeting:@"SMA_N3"] intValue];
    int SMA_N4 = [[LTUtils getSeting:@"SMA_N4"] intValue];
    int SMA_N5 = [[LTUtils getSeting:@"SMA_N5"] intValue];
    
    int MACD_N1 = [[LTUtils getSeting:@"MACD_N1"] intValue];
    int MACD_N2 = [[LTUtils getSeting:@"MACD_N2"] intValue];
    int MACD_P = [[LTUtils getSeting:@"MACD_P"] intValue];
    int RSI_N1 = [[LTUtils getSeting:@"RSI_N1"] intValue];
    int RSI_N2 = [[LTUtils getSeting:@"RSI_N2"] intValue];
    int RSI_N3 = [[LTUtils getSeting:@"RSI_N3"] intValue];
    int BOLL_K = [[LTUtils getSeting:@"BOLL_K"] intValue];
    int BOLL_N = [[LTUtils getSeting:@"BOLL_N"] intValue];
    ...
    // MA值
    CGFloat MA5 = [FMStockIndexs createMAWithPrices:array MA:SMA_N1 Index:i];
    CGFloat MA10 = [FMStockIndexs createMAWithPrices:array MA:SMA_N2 Index:i];
    CGFloat MA20 = [FMStockIndexs createMAWithPrices:array MA:SMA_N3 Index:i];
    CGFloat MA60 = [FMStockIndexs createMAWithPrices:array MA:SMA_N4 Index:i];
    CGFloat MA120 = [FMStockIndexs createMAWithPrices:array MA:SMA_N5 Index:i];
    ...
    // 计算每天的MACD值
    NSMutableDictionary *MACD = [FMStockIndexs getMACD:array
                                               andDays:i
                                           DhortPeriod:MACD_N1
                                            LongPeriod:MACD_N2
                                             MidPeriod:MACD_P];
    ...
    // 计算RSI值
    [FMStockIndexs getRSIWithDay:i Key:@"RSI_1" Number:RSI_N1 Data:array];
    [FMStockIndexs getRSIWithDay:i Key:@"RSI_2" Number:RSI_N2 Data:array];
    [FMStockIndexs getRSIWithDay:i Key:@"RSI_3" Number:RSI_N3 Data:array];
    ...
    //传入之前和当前的数据计算EMA值
    NSMutableArray *list = [NSMutableArray arrayWithObjects:[array objectAtIndex:prei],item,nil];
    ...
    if(array.count<EMA_Tianji_Cycle_2){
        EMA_Tianji_Cycle_1 = (int)array.count - sub;
        EMA_Tianji_Cycle_2 = (int)array.count;
    }
    ...
    list = nil;
    // 计算每天的BOLL值
    NSMutableDictionary *BOLL = [FMStockIndexs getBOLLWithDay:i K:BOLL_K N:BOLL_N Data:array];
