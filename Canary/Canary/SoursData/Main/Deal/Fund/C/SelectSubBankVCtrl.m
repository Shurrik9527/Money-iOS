...
@implementation SubBankMO

@end

@interface SelectSubBankVCtrl ()<UISearchBarDelegate>
...
@implementation SelectSubBankVCtrl

- (void)viewDidLoad {
    [super viewDidLoad];
    
    [self navPopBackTitle:@"选择支行"];
...
- (void)loadData {
    WS(ws);
    [RequestCenter reqBranchBankList:_cityCode bankCode:_bankCode finsh:^(LTResponse *res) {
        if (res.success) {
            NSArray *arr = [SubBankMO objsWithList:res.resArr];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    static NSString *identifier=@"SubBankCellID";
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(selectSubBank:)]) {
        [_delegate selectSubBank:mo];
        [self popVC];
    }
}
