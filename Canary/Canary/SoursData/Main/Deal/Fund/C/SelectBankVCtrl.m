...
@implementation BankMO

@end

@interface SelectBankVCtrl ()<UISearchBarDelegate>
...
@implementation SelectBankVCtrl

- (void)viewDidLoad {
    [super viewDidLoad];
    
    [self navPopBackTitle:@"选择银行"];
...
- (void)loadData {
    WS(ws);
    [RequestCenter reqBankList:^(LTResponse *res) {
        if (res.success) {
            NSArray *arr = [BankMO objsWithList:res.resArr];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    static NSString *identifier=@"BankCellID";
    ...
    [cell.imageView lt_setImageWithURL:mo.icon];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(selectBank:)]) {
        [_delegate selectBank:mo];
        [self popVC];
    }
}
