...
@interface SelectCityVCtrl ()<UISearchBarDelegate>
...
@implementation SelectCityVCtrl

- (void)viewDidLoad {
    [super viewDidLoad];
    
    [self navPopBackTitle:@"选择城市"];
...
- (void)loadData {
    WS(ws);
    [RequestCenter reqCityList:_provinceCode finsh:^(LTResponse *res) {
        if (res.success) {
            NSArray *arr = [CityMO objsWithList:res.resArr];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(selectCity:)]) {
        [_delegate selectCity:mo];
        [self popVC];
    }
}
