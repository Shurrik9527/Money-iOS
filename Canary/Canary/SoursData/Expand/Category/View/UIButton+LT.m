...
static const void *LitButtonBlockKey = &LitButtonBlockKey;
...
- (void)addAction:(TouchedBlock)touchHandler {
    objc_setAssociatedObject(self, LitButtonBlockKey, touchHandler, OBJC_ASSOCIATION_COPY_NONATOMIC);
    [self addTarget:self action:@selector(actionTouched:) forControlEvents:UIControlEventTouchUpInside];
}
- (void)actionTouched:(UIButton *)btn{
    TouchedBlock block = objc_getAssociatedObject(self, LitButtonBlockKey);
    if (block) {
        block(btn.tag);
    }
}
...
- (void)setTitleFontSize:(CGFloat)size {
    self.titleLabel.font = [UIFont fontOfSize:size];
}
