...
#import "SelfList+CoreDataProperties.h"

@implementation SelfList (CoreDataProperties)

+ (NSFetchRequest<SelfList *> *)fetchRequest {
	return [NSFetchRequest fetchRequestWithEntityName:@"SelfList"];
}
...
+(BOOL)AddData:(MarketModel *)model{
    ...
    SelfList *home=[NSEntityDescription insertNewObjectForEntityForName:@"SelfList" inManagedObjectContext:kCoreDataContext];
...
// 删除
+(void)DeleteAll{
    NSFetchRequest *request = [NSFetchRequest fetchRequestWithEntityName:@"SelfList"];
    
    NSArray *arr = [kCoreDataContext executeFetchRequest:request error:nil];
    
    for (SelfList*p in arr) {
