//
//  ForeignCurrencyList+CoreDataProperties.m
//  Canary
//
...
#import "ForeignCurrencyList+CoreDataProperties.h"

@implementation ForeignCurrencyList (CoreDataProperties)

+ (NSFetchRequest<ForeignCurrencyList *> *)fetchRequest {
	return [NSFetchRequest fetchRequestWithEntityName:@"ForeignCurrencyList"];
}
...
+(BOOL)AddData:(MarketModel *)model{
    ...
    ForeignCurrencyList *home=[NSEntityDescription insertNewObjectForEntityForName:@"ForeignCurrencyList" inManagedObjectContext:kCoreDataContext];
...
// 删除
+(void)DeleteAll{
    NSFetchRequest *request = [NSFetchRequest fetchRequestWithEntityName:@"ForeignCurrencyList"];
    
    NSArray *arr = [kCoreDataContext executeFetchRequest:request error:nil];
    
    for (ForeignCurrencyList*p in arr) {
...
+(void)Delete:(MarketModel *)model{
    ...
    for (int i = 0; i < arr.count; i++) {
        ...
        if ([string isEqualToString:[[arr objectAtIndex:i] symbol]]) {
            ...
            for (ForeignCurrencyList*home in arr) {
